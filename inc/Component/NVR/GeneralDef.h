...
typedef struct STRU_LOCAL_PARAM
{
	...
	STRU_LOCAL_PARAM()
	{
		...
		sprintf_s(chDownLoadPath, 100, "%s","C:\\DownLoad");
		sprintf_s(chPictureSavePath, 100, "%s", "C:\\Picture");
		sprintf_s(chRemoteCfgSavePath, 100, "%s", "C:\\SaveRemoteCfgFile");
		sprintf_s(chClientRecordPath, 100, "%s", "C:\\mpeg4record\\2008-04-30");
		sprintf_s(chIPServerIP, 100, "%s", "172.7.28.123");
...
typedef struct _NET_DVR_FindRemoteFile
{
	int nCh;
	CTime ctDateStart;
	CTime ctDateEnd;
	CTime ctTimeStart;
	CTime ctTimeEnd;
}NET_DVR_FindRemoteFile, *LPNET_DVR_FindRemoteFile;

typedef struct _NET_DVR_RemoteFile
{
	char szFileName[MAX_PATH];
	char szFileSize[MAX_PATH];
	char szDateStart[MAX_PATH];
	char szDateEnd[MAX_PATH];
	char szTimeStart[MAX_PATH];
	char szTimeEnd[MAX_PATH];
}NET_DVR_RemoteFile, *LPNET_DVR_RemoteFile;

typedef struct _NET_DVR_PlayInfo
{
	BYTE* pY;
	BYTE* pU;
	BYTE* pV;
	int nWidth;
	int nHeight;
}NET_DVR_PlayInfo, *LPNET_DVR_PlayInfo;
